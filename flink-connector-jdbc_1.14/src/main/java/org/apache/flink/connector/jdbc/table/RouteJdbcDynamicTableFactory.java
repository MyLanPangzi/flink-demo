...
public class RouteJdbcDynamicTableFactory implements DynamicTableSinkFactory {

    public static final String IDENTIFIER = "route-jdbc";
    ...
    private JdbcDmlOptions getJdbcDmlOptions(JdbcConnectorOptions jdbcOptions, ResolvedSchema schema) {
        String[] keyFields =
                schema.getPrimaryKey().map(pk -> pk.getColumns().toArray(new String[0])).orElse(null);

        String[] cols = schema.getColumnNames().toArray(new String[0]);
